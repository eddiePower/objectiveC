...
{
    //Fetch the MonsterData entity from coreData store.
...
{
    NSError* error;
    //create Object(monsterData) from JSON data passed into method(data)
    ...
    //IF result is an array then make object
    if([result isKindOfClass:[NSArray class]])
    {
        NSArray* monsterArray = (NSArray*)result;
        NSLog(@"Found %lu new monsters!", (unsigned long)[monsterArray count]);
        
        //Create a NSDict of Monsters from MonsterArray made up of JSON made objects.
        for(NSDictionary* monster in monsterArray)
        {
            //Insert new object into ManagedObjContext space
            ...
            //set that objects name and type to store in managed object space.
        ...
        //Save data to managedObjectContext and check for errors.
        if(![self.managedObjectContext save:&error])
        {
            NSLog(@"Could not save downloaded monster data:\n%@", error.userInfo);
        }
    }
    else    //otherwise the json format was not what was expected (not an Array object)
    {
